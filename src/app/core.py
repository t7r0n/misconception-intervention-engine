...
def init_demo(root: Path | None = None, seed: int = 42, records: int = 96) -> dict[str, Any]:
    ...
    if domain.should_enable(config):
        domain_summary = domain.generate_domain_fixtures(fixtures)
...
def ingest(root: Path | None = None, fixture_dir: Path | None = None) -> dict[str, Any]:
    ...
    domain_cases = domain.load_domain_cases(source_dir)
    domain_count = domain.ingest_domain_cases(db_path, domain_cases)
    summary = {
        "records": len(records),
        "domain_cases": domain_count,
...
def analyze(root: Path | None = None) -> dict[str, Any]:
    ...
    domain_analysis = domain.analyze_domain(project_root)
    ...
    if domain_analysis:
        report_lines.extend(["", "## Domain-Specific Decisions", ""])
        for case in domain_analysis["cases"][:8]:
            report_lines.append(
                "CLAIM: "
                f"{compact(case['subject'])} -> {case['recommendation']} "
                f"because {case['reason']} with score={case['score']}. "
                f"[EVID: {case['evidence_id']}]"
            )
    if domain_analysis:
        report_lines.extend(["", "## Domain-Specific Decisions", ""])
        for case in domain_analysis["cases"][:8]:
    ...
    if domain_analysis:
        packet.extend(["", "## Domain-Specific Evidence", ""])
        for case in domain_analysis["cases"]:
            packet.append(f"- {case['evidence_id']}: {case['evidence']} -> {case['notes']}")
    if domain_analysis:
        packet.extend(["", "## Domain-Specific Evidence", ""])
        for case in domain_analysis["cases"]:
            packet.append(f"- {case['evidence_id']}: {case['evidence']} -> {case['notes']}")
...
def valid_evidence_ids(root: Path) -> set[str]:
    ...
    ids.update(domain.evidence_ids_from_db(root))
    return ids
...
def verify(root: Path | None = None) -> dict[str, Any]:
    ...
    checks = [
        ...
        ("domain_cases_present", bool(analysis.get("domain", {}).get("cases"))),
    ]
...
<main>
  ...
  <section class="panel">
    ...
    <table>
      <thead><tr><th>Subject</th><th>Recommendation</th><th>{{ domain_analysis.primary_label }}</th><th>Evidence</th></tr></thead>
      <tbody>
      {% for case in domain_analysis.cases %}
      <tr>
        <td>{{ case.subject }}</td>
        <td>{{ case.recommendation }}</td>
        <td>{{ case.score }}</td>
...
def export_demo_pack(root: Path | None = None) -> dict[str, Any]:
    ...
    domain_summary = ""
    domain_path = outputs / domain.ANALYSIS
...
- Layer: {domain_data['label']}
- Cases: {domain_data['case_count']}
- Average score: {domain_data['average_score']}
- Missing or blocked: {domain_data['missing_or_blocked_count']}

The domain layer converts synthetic but realistic workflow cases into scored, evidence-backed decisions and explicit blocks/reviews for unsupported or risky outputs.
"""
